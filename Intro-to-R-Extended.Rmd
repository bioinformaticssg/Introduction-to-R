...
```{r}
# Assign the row names of the dataframe to be the ID's found in the column 
# GeneID
...
# conditions. In this case, the requirements are that in each row, all three of 
# the columns have a minimum of 1 read.
...
# Our conditions duplicated the same conditions when we applied all() to the 
# data. Thus, we expect the dimensions of the filtered data to be the 
# same: rows (565) and columns (3).
dim(filtered_data)
```
